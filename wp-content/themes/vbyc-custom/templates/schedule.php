...
<?php 
    ...
    if ($sort_by == "date") {
        ...
        foreach( $repeater as $i => $row ) {
            $order['date_start'][ $i ] = $row['date_start'];
            $order['session_name'][ $i ] = $row['session_name']; 
        }

        // multisort
        array_multisort( $order['date_start'], $order['session_name'], SORT_ASC, $repeater );
                           ...
                           <ul id="sidenav" class="sidenav nav sidenav-list list-unstyled">
                            ...
                            if ($sort_by == "date") {
                                ...
                                foreach( $repeater as $i => $row ): 
                                    ...
                                    $link_id = convertToLinkable($session_name);
                                ?>

                                    <li class="item" role="presentation">
                                        <a href="#session-<?=$link_id?>" class="link"><?=$session_name?></a>
                                    </li>
                                <?php
                            ...
                            <?php include("".$_SERVER["DOCUMENT_ROOT"]."/wp-content/themes/vbyc-custom/inc/mini-gallery.php"); ?>
                                ...
                                if ($sort_by == "date") {
                                    ...
                                    foreach( $repeater as $i => $row ): 
                                        ...
                                        $link_id = convertToLinkable($headline);
                                    ...
                                    <h2 id="session-<?=$link_id?>" class="h2 heading" >
                                        <span class="nowrap"><?=$headline?></span> | 
                                        <span class="date nowrap"><?=$display_dates?></span> 
                                    </h2>
                                    <p id="session-<?=$link_id?>">
                                     
                                        <strong><span class="ages nowrap"><?=$group_current_label?></span>  
                                                                                <?php if ($cost) { ?>| $<?=$cost?> <? } ?></strong>
                                    </p>
